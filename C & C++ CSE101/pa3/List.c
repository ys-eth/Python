...
// Returns true if Lists A and B are in the same state else returns false.
bool equals(List A, List B) {
    ...
    /*for (a = A->front, b = B->front; a != NULL || b != NULL; a = a->next, b = b->next) {
        if (a->data != b->data) {
            return false;
        }
    }
...
// Prints to the file point to by out, a string representation of L consisting of a space separated sequece of integers, with front on the left.
void printList(FILE *out, List L) {
    Node j = L->front;
    for (int i = 0; i < length(L); i++) {
        fprintf(out, "%d ", j->data);
        j = j->next;
    }
}
